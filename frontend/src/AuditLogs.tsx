...
export default function AuditLogView({
  ...
  return (
    <div className="w-full bg-[#FBF5DD] border border-[#E7E1B1] rounded-xl p-6 shadow-sm text-[#0D530E]">
      <div className="border-b border-[#E7E1B1] pb-4 mb-6">
        <div>
          <h2 className="text-lg font-bold tracking-tight text-[#0D530E] flex items-center gap-2">
            Cluster Audit Log History
          </h2>
          <p className="text-xs text-[#306D29] mt-0.5">
            Historical timeline of core engine lifecycle actions
          </p>
        </div>

        <div className="flex items-center gap-2 mt-4 font-mono text-xs w-full">
          <button
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between bg-[#E7E1B1]/10 border border-[#E7E1B1]/60 p-3 rounded-xl shadow-2xs w-full mb-4">
            ...
            <select
              ...
              className="px-2.5 py-1.5 text-xs bg-white/80 border border-[#E7E1B1] rounded-lg text-[#0D530E] focus:outline-none focus:border-[#306D29] cursor-pointer font-medium shadow-sm"
            >
            ...
            <button
              ...
              disabled={loading}
              className="px-3 py-1.5 bg-[#306D29] hover:bg-[#0D530E] active:bg-[#0D530E]/90 disabled:opacity-40 text-[#FBF5DD] text-xs min-w-[150px] font-semibold rounded-lg transition-all shadow-sm cursor-pointer"
            >
          ...
          {/* table */}
          <div className="overflow-x-auto rounded-lg border border-[#E7E1B1] max-h-[520px] overflow-y-auto bg-white/50 backdrop-blur-sm">
            <table className="w-full text-left border-collapse table-auto">
              <thead>
                <tr className="bg-[#E7E1B1]/40 border-b border-[#E7E1B1] text-[#0D530E] font-mono text-[11px] uppercase tracking-wider sticky top-0 z-10 backdrop-blur-md">
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 font-medium whitespace-nowrap">
                          <span
                            className={`px-2 py-0.5 rounded-md text-[10px] font-bold font-mono tracking-wide uppercase ${isWarn ? "bg-red-100 text-red-800 border border-red-200" : "bg-[#306D29]/10 text-[#306D29] border border-[#306D29]/20"}`}
                          >
                            {isWarn ? "Warn" : "Info"}
                          </span>
                        </td>
                        ...
                        <td
                          className="px-4 py-3 text-[#0D530E] font-semibold font-mono max-w-[200px] truncate"
                          title={log.pod_name}
                        >
                          {log.pod_name || "cluster-level"}
                        </td>
                        <td className="px-4 py-3 text-[#0D530E]/80 pr-6 leading-relaxed select-text font-sans break-words font-medium">
                          {log.message}
                        </td>
          ...
          {/* pagination */}
          <div className="flex items-center justify-between border border-[#E7E1B1] bg-[#E7E1B1]/20 px-4 py-3 rounded-b-lg text-xs font-medium mt-0">
            <div className="text-[#306D29]">
              Showing{" "}
              <span className="font-bold text-[#0D530E]">{logs.length}</span> of{" "}
              <span className="font-bold text-[#0D530E]">{totalItems}</span>{" "}
              audit events
            </div>
            <div className="flex items-center gap-4">
              <span className="text-[#306D29]">
                Page{" "}
                <span className="font-bold text-[#0D530E]">{currentPage}</span>{" "}
                of{" "}
                <span className="font-bold text-[#0D530E]">{totalPages}</span>
              </span>
              <div className="flex gap-2">
                <button
                  onClick={() => handlePageChange(currentPage - 1)}
                  disabled={currentPage === 1 || loading}
                  className="px-3 py-1 bg-white border border-[#E7E1B1] text-[#0D530E] hover:bg-[#FBF5DD] disabled:opacity-40 rounded-md transition-all font-semibold shadow-sm cursor-pointer disabled:cursor-not-allowed"
                >
                  {"<"} Prev
                </button>
                <button
